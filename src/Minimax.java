...
public class Minimax extends Thread {
    ...
    private static int PLAYER1_MOVE = currentPlayerAI.playUser;  //red player
    private static int PLAYER2_MOVE = currentPlayerAI.playComp; //blue player
    ...
    static int[][] gameBoard = ConnectFour.positions;
    private int num;
    private int mychip;
    ...
    public static boolean isLegalMove(int row, int col) {
    	...
    	if (row < ROW_SIZE && col < COL_SIZE) {
    		...
    		if (gameBoard[row][col] == OPEN_SQUARE) {
    		    ...
    		    if (gameBoard[row+1][col] != OPEN_SQUARE) return true;
    		}
    	    }
    	}
    	return false;
	...
	public void run() {
		//get the game board
		gameBoard = ConnectFour.positions;
